...
import { Pool as LiquidityPoolStore } from "../../generated/schema";
...
const poolDailySnapshot = getOrCreatePoolDailySnapshots(
    pool.id,
    vaultAddress,
    block
);
const pooltHourlySnapshot = getOrCreatePoolHourlySnapshots(
    pool.id,
    vaultAddress,
    block
  );
